...
#### 7.9 BrandModelCard（快惠选专用）

```
整卡：bg:#F5F5F5, radius:4px
商品图：54×54px, radius:4px

内部文字规格：
  品牌机型名称  12px/400/#000, letter-spacing:0.22
  热销榜 badge  bg:rgb(255,249,211) + 红三角 SVG rgb(255,72,60)
                文字：9px/300/#000
  品牌·第N名   9px/300/#000
  卖点          9px/300/#666, 0.5px 黑色细线分隔
  ¥ 符号        8px/500
  价格数字      12px/500
  价格副文案    9px/300/#000, 0.5px #BBB 分隔
  行动按钮      h24, radius:2（⚠️ 不是 pill！），10px/400/#000
```
...
```
使用一张通用 PNG 占位图
文字：标题 18px/500/#111 + 辅助文本 14px/400/#666，2行内居中
无彩色插画 / 3D图标 / 装饰性插图
操作按钮（如有）：Button size="md" variant="strong"
```
...
> 以下仅在**营销场景**可用，工具型场景（搜索/列表/商详/订单/表单/我的/设置等）禁令照旧。

**触发营销场景的条件（满足任一）：**
1. 页面类型属于：活动会场 / 节点页（双11/618/周年）/ 抽奖 / 倒计时活动入口 / 大促集合页
2. 用户明确要求：营销氛围 / 促销活动 / 造节 / 会场 / 强冲击力 / 视觉张力
3. PRD 标记 `type: marketing/campaign/活动`

| 默认禁用项 | 营销场景 | 限制 |
|---|---|---|
| 装饰渐变 | ✓ 允许 | 仅活动 banner/倒计时/抽奖入口；商品卡/列表仍禁 |
| 大面积品牌红 | ✓ 允许 | 营销模块可做大底色/大色块 |
| 营销文案 | ✓ 允许 | 禁止绝对化："全网最低/独家/史上最便宜/错过等一年" |
| 感叹号 | ✓ 允许 | 全模块不超过 2 处 |
| 投影/装饰发光 | ✓ 允许 | 仅活动模块 |
| emoji | ⛔ 仍禁 | 性能+一致性，不软化 |
| 毛玻璃/backdrop-filter | ⛔ 仍禁 | 不软化 |
